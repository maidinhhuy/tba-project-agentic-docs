...
inputDocuments:
  ...
  - backend/template (starter template — auth pattern tham khảo)
...
_Tài liệu này được xây dựng qua collaborative discovery. Phiên 2026-06-11 đã hoàn thành Step 1 (Init) và Step 2 (Context Analysis + Party Mode). Step 3 (Starter Template) đã incorporate backend/template với JWT auth pattern. Tiếp tục Step 4+ ở phiên mới._
...
15 FRs trong 5 nhóm: Account Management, Project Submission, Project Tracking (Customer Portal), Admin Project Management, và Email Notifications. Core flow: Customer submit project → Admin quản lý lifecycle qua 8-state machine → Email notification tự động ở key transitions. Không có self-signup, không có file upload, không có real-time features ở v1.
...
- **Auth:** JWT stateless — access token (15 min) + refresh token (7 days, stored in `refresh_tokens` table) — code pattern tham khảo từ `backend/template` (xem AD-01)
...
**Không chọn:** ~~httpOnly cookie + Spring Session JDBC~~ (quyết định ban đầu — đã đổi sau khi xác nhận dùng `backend/template` làm auth pattern reference).

**Lý do đổi sang JWT:**
- `backend/template` đã có auth stack hoàn chỉnh (JwtTokenProvider, JwtAuthenticationFilter, LoginAttemptService, TokenManagementService) — tái dùng trực tiếp, không viết lại từ đầu
...
### AD-04: Project Structure — Monorepo

**Quyết định:** 1 Git repo, 2 folders (`/frontend`, `/backend`).

**Không chọn:** Separate repos.

**Lý do:** Solo operation, không có overhead sync 2 repos, shared types/contracts dễ maintain hơn, một CI pipeline.
...
### AD-12: Account Creation Flow (Không có self-registration)

**Quyết định:** Không có public `/register` endpoint. Admin tạo tài khoản Customer trực tiếp qua Admin Dashboard — hoặc qua một Admin-only API endpoint.

**Không chọn:** Self-signup flow (template có `RegisterUserService` + email verification — không dùng ở v1).

**Lý do:** PRD §2.2 Non-Users nêu rõ: "Người dùng tự đăng ký — v1 chỉ hỗ trợ tài khoản do admin tạo."

**Adaptation từ template:**
- `RegisterUserService` được giữ lại nhưng endpoint `/api/auth/register` đổi từ `permitAll()` thành `hasRole("ADMIN")`
- Email verification flow (EmailVerificationService, `/api/verification/*`) bỏ qua ở v1 — admin set `is_active=true` và `is_email_verified=true` khi tạo account
- Admin tạo account với password tạm thời → thông báo Customer qua email ngoài hệ thống (v1 manual)
...
- ✅ Step 3: Starter Template — `backend/template` incorporated. AD-01→JWT, AD-12 admin-only account creation.
