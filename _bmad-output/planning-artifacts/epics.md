...
**And** `Makefile` tại root của FE repo định nghĩa: `test` target (`npm test`), `dev` target (`npm run dev`), `build` target (`npm run build`). Agent chạy test đồng nhất qua `make test` (AD-19).
